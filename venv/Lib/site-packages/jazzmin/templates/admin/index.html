                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group float-right">
                                            {% if model.add_url %}
                                                <a href="{{ model.add_url }}" class="btn btn-xs {{ jazzmin_ui.button_classes.success }} addlink" style="font-weight: bolder; font-weight: bolder;">{% trans 'Add' %}</a>
                                            {% endif %}
                                            {% if model.url %}
                                                {% if model.view_only %}
                                                    <a href="{{ model.url }}" class="btn btn-xs {{ jazzmin_ui.button_classes.info }} viewlink" >{% trans 'View' %}</a>
                                                {% else %}
                                                    <a href="{{ model.url }}" class="btn btn-xs {{ jazzmin_ui.button_classes.info }} changelink" style="font-weight: bolder; font-weight: bolder;">{% if model.custom %}{% trans 'Go' %}{% else %}{% trans 'Edit' %}{% endif %}</a>
                                                {% endif %}
                                            {% endif %}
                                        </div>
                                    </td>
                    ...
                    <div class="timeline" >
                        {% for entry in admin_log %}
                            <div>
                                ...
                                <div class="timeline-item" style="background-color: aliceblue;  width: 96%;">
                                    ...
                                    <h3 class="timeline-header no-border">
                                        {% if entry.is_deletion or not entry.get_admin_url %}
                                            {{ entry.object_repr }}
                                        {% else %}
                                            <a href="{{ entry.get_admin_url }}">{{ entry.object_repr|truncatewords:"9" }}</a>
                                        {% endif %}
